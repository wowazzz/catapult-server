...
namespace catapult { namespace plugins {
    ...
    NODESTROY std::string pricePublisherAddress = "";
    ...
    double getCoinGenerationMultiplier(uint64_t blockHeight, bool rollback) {
        if (blockHeight % multiplierRecalculationFrequency > 0 && !areSame(currentMultiplier, 0) && !rollback) // recalculate only every 720 blocks
